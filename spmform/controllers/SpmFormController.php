 ...
 */
class SpmFormController extends BaseController
{
	/**
	...
	public function actionSendMessage()
	{
		...
		// Zusätzliche Felder einbeziehen
		// -----------------------------------------------------------
		$configs = $message->attributeConfigs;

		foreach ($configs as $key => $val) {
			if (!in_array($key, $exclude_fields)) {
				$post_val = craft()->request->getPost($key);
				if (!empty($post_val)) {
					$postedMessage[$val['label']] = $post_val;
					$message[$key] = $post_val;
				}
			}
		}
		...
		if ($postedMessage)
		{
			if (is_array($postedMessage))
			{
				...
				// Actually, my body can be empty, I think.
		...
		if ($message->validate())
		{
			...
			if (!$this->validateHoneypot($settings->honeypotField) || craft()->spmForm->sendMessage($message))
			{
